...
for c = 1:38
    for i = 1:70
        ftgra(i,:,c) = fft(smg(i,:,c));
        ftgra(i,:,c) = sqrt(ftgra(i,:,c).^2/real(sum(ftgra(i,:,c)).^2)); % normalize so
                        % that the frequencies for each roi/scan sum to one
        kde(i,:,c) = ksdensity(ftgra(i,:,c)); % use it to compute kernel estimation
    end
end
...
for j = 1:70
    for a = 1:38
        for b = 1:38
            ...
            %to
            H(j,a,b) = norm(ftgra(j,:,a) - ftgra(j,:,b))/sqrt(2);
